...
export default function GalleryHeader({ prop }: { prop: keyof typeof PROPS }) {
    ...
    const endpoint =
        ENDPOINT_MAP[location.pathname as keyof typeof ENDPOINT_MAP];
            ...
            <div className="flex items-center gap-3">
                {count > 0 && (
                    ...
                    >
                        ...
                        <span>Delete</span>
                        <Badge className="absolute -top-2 -right-2 h-5 w-5 rounded-full text-xs flex items-center justify-center bg-white text-red-600 font-bold shadow-md border border-gray-600">
                            {count}
                        </Badge>
                    </Button>
                )}
